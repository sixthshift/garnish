...
test("getDb caches the handle and does not re-seed", async () => {
  ...
  const b = getDb();
  expect(b).toBe(a);
  expect(unitRepository(a).list()).toHaveLength(DEFAULT_UNITS.length);
});

test("closeDb forgets the handle; the next getDb reopens the same file", async () => {
  const a = getDb();
  unitRepository(a).create({ name: "handful", pluralName: "handfuls", abbreviation: "hf" });
  closeDb();
  const b = getDb();
  expect(b).not.toBe(a);
  expect(
  ...
  expect(unitRepository(b).list()).toHaveLength(DEFAULT_UNITS.length + 1);
});
