...
describe("runRestyle", () => {
  ...
  test("a missing part is malformed: the pairing is verified, never assumed", async () => {
    ...
    expect(caught).toBeInstanceOf(AiImportError);
    expect((caught as AiImportError).kind).toBe("malformed");
    expect((caught as AiImportError).message).toMatch(/1 part where the recipe has 2/);
  });
  ...
  test("matchParts passes a matching answer and the runner's own failures come through untouched", async () => {
    ...
    const boom: AiRunner = async () => {
      throw new AiImportError("timeout", "The model took too long to answer.");
    };
