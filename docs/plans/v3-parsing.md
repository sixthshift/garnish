# Plan: stage 3, ingredient parsing and parts (complete)

Stage 3 of the garnish plan, written to be executed one task per loop iteration with no human present, and complete as of 2026-09-12. Decisions live in [decisions.md](../decisions.md), shape in [architecture.md](../architecture.md), boundaries in [scope.md](../scope.md). Stage 1 is [v1-foundations.md](v1-foundations.md), stage 2 [v2-ui.md](v2-ui.md); the live plan is [plan.md](../plan.md). Kept for the record: the task list as written, and the Log of what each iteration actually did.
...
- **M17.4** `unitText` echoes the unit word as written on a match, is empty on a miss (the token is handed on to the food), and after the unit backtrack carries the proposed unknown unit ("2 sprigs rosemary" -> `unitText: "sprigs"`). Mealie proposes a unit only where one was recognisably used, which is what this does. If M17.5's review step needs a "create this unit" name in more cases, widen the rule there rather than in the parser.

## Log

_(one line per iteration: date, task id, outcome, model)_

2026-09-11  M17.1  done  83d2e6d  opus  parseQuantity reads leading amounts: integers, decimals, ASCII and vulgar fractions, a/an, Cooklang `=`, low-of-range
2026-09-11  M17.2  done  86759e6  sonnet  parseUnit longest-matches name/plural/abbreviation case-insensitively, plus a case-sensitive alias map (T vs t) for spellings the seed lacks
2026-09-11  M17.3  done  1b0ca61  sonnet  parseFood anchors a longest-match over name/plural/aliases on the comma- and parenthetical-stripped head; a partial or missing match lands in foodText rather than dropping text
