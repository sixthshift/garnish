# Plan: stage 5, cooking from it (complete)

Stage 5 of the garnish plan, written to be executed one task per loop iteration with no human present, and complete as of 2026-09-12. Decisions live in [decisions.md](../decisions.md), shape in [architecture.md](../architecture.md), boundaries in [scope.md](../scope.md). Stage 1 is [v1-foundations.md](v1-foundations.md), stage 2 [v2-ui.md](v2-ui.md), stage 3 [v3-parsing.md](v3-parsing.md), stage 4 [v4-editor.md](v4-editor.md); the live plan is [plan.md](../plan.md). Kept for the record: the task list as written, and the Log of what each iteration actually did.

Run in three parallel lanes from M25.2 on, each task in its own git worktree, cherry-picked onto main and re-gated there; one conflict, in the view route between M25.6 and M26.3, resolved by hand. Jason's review at the end: busier, not better. Stage 6 is the correction.
...
- [x] **M24.7 Leave the 3xl column behind.** M24.1 widened only the view page and only from `lg`, so between `md` and `lg` the two-column grid still squeezes into a 768px container that also loses 224px to the side nav, and the editor never widened at all. The view route goes to `max-w-6xl` at every width (below 1152px the cap never binds, so a tablet gets the whole content area); `/recipes/new` and `/recipes/$slug/edit` go to `max-w-5xl`, so the inline ingredient row's five fields stop wrapping. Cook mode keeps `max-w-3xl`: one card of large type at roughly 35 characters a line is the right measure for reading across a bench, and that is a deliberate exception, noted in the route's header comment. Check: the M24.1 width test is updated to the new classes; a grep finds `max-w-3xl` only in cook.tsx among the recipe routes.
...
2026-09-12  M27.6  done  3de03ff  sonnet  the toolbar's Edit as JSON button becomes a one-item Menu at its end; Menu.Item passes data-testid through and the form takes a test-only jsonMenuOpen seam. Cherry-picked from task/M27.6
2026-09-12  M27.5  done  736aaaf  opus  a pencil on each ingredient and step row (hover from md, long press below via useLongPress) opens a sheet over that one row; QuickEditProvider carries the stored unscaled document and Save writes it whole through updateRecipe with every id kept, so ticks survive. The merged summary list gets no pencil: a merged row has no single stored row behind it
2026-09-12  M24.7  done  47f4734  sonnet  the view route is max-w-6xl at every width with the method column capped at max-w-prose, new and edit are max-w-5xl, and cook keeps max-w-3xl with the reason in its header comment. Added after Jason asked why 3xl was still there: M24.1 had widened only the view page and only from lg
