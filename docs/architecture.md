...
- New recipe: `/recipes/new` opens on a paste box, not a blank form (decisions.md row 52). `splitRecipe` (`src/domain/splitRecipe.ts`) takes a title off the front of the pasted text, then divides the rest on its headings — Ingredients / Method — or, with none, on the shape of the lines: the leading run that reads like ingredients, the prose after it. Bullets and numbered step markers are stripped. Each ingredient line goes through `parseIngredient` and the whole thing is reviewed with the M17.5 review rows before Create builds a `RecipeDraft`; only the foods and units the reviewer approved are written, through `findOrCreateFood`/`findOrCreateUnit`. `?blank` skips straight to the empty editor, which is what "Start blank" navigates to. Rules-based throughout — no network, no AI; the `claude -p` import (scope.md, Later) will replace the split and keep the review.
