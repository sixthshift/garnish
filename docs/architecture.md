...
src/
  ...
  domain/import/    a module: index.ts is its whole surface and nothing outside the folder imports anything else in it (test/modules.test.ts). The surface is the Importer — `new Importer({ fetchPage, model }).import(source)`, where a source is `{ kind: "url" }`, `{ kind: "text" }` or `{ kind: "file" }` and the importer chooses the sequence — its ImportError, the result types, the recipe's JSON Schema for the model port, and the review-row builders. Inside: importer.ts (the class), errors.ts, result.ts (ImportedRecipe, ImportSource), scraped.ts (ScrapedRecipe and the schema.org field parsers), extract.ts (a page in, a result out), model.ts (the prompt, the anchor, the answer parser), check.ts (the answer against its anchor)
