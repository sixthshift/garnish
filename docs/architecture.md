...
src/
  ...
  db/models/    one folder per domain, each with schema.ts (its Drizzle tables) and repo.ts (its repository: the factory by name — `tagRepository(db)`, a closure that is a constructor in all but syntax — and as the default export the same repository over the application database — a lazy object built on first use, one per handle; shopping/ composes its from a context.ts, read.ts and write.ts beside it); columns.ts holds the shared column defaults and the conventions they all follow
  db/models/recipe/    schema.ts: recipe, part, ingredient, step, recipe_note, recipe_tag — one aggregate, written as one document. repo.ts: the repository, the collection — read top-down: the query grammar (`RecipeQuery`, a discriminated union: the faceted filter, or by food, unit, tag, source URL or exact name), reference resolution and hydration, reading a document, writing a document, answering a query with summary rows, then `get`, `getById`, `query`, `create`, `ref(id)` handing out a record, and `remove`. summary.ts: how a RecipeSummary is made — the columns a query selects, the tag and ingredient-preview reads that finish a row — pure over the handle. record.ts: `recipeRecord(storage, id)`, one recipe and the operations done to it (`replace`, `favourite`, `rate`, `setImage`, `restyle`, `restore`, `hasStep` and `setStepImage` scoped to its parts, `subRecipes` following its foods' `recipe_id`) plus the step swap a restyle makes; `RecipeStorage` is the handle and document helpers the repository lends it. list.ts: the list filter and sort grammar, pure over the Drizzle handle
...
- **Server functions** (`createServerFn`) for everything the app itself calls, one file per resource in `src/server/fns/`. Input validated with the shared zod schemas. Reads use `method: 'GET'` so they are cacheable; writes are `POST`.
  ...
  - `foods`, `units`, `aisles`, `tags`: `list({ q? })`, `create`, `update`, `delete`, `findOrCreate({ name })` each, e.g. `listUnits`, `findOrCreateTag`. Input schemas in `src/domain/reference/reference.ts`.
    - `usingFood`, `usingUnit`, `usingTag` list the recipes a delete would touch (`recipes.query({ by: "food" | "unit" | "tag", id })`); the confirm dialog shows them.
...
- **Server routes** under `/api/*` only for callers outside the app, and later import hooks. Field names follow Mealie where the concept exists.
  ...
  - `POST /api/recipes/:id/steps/:stepId/image` and `GET /api/images/steps/:file`: the same pair again for one method step's photo, addressed by the recipe that owns the step (another recipe's step id is 404), under `images/steps/` (M35.1). The file is named for the step id, and the name rides in the recipe document beside the step's text, so the save that re-inserts every step keeps it. A step the recipe has never saved has no row and is a 404, which is what the editor reports as "save first".
...
- One process-wide handle (`getDb()`), opened lazily, migrated and seeded once, retried on failure. It is synchronous — `bun:sqlite` is, and the bundled migrations need no IO — so a repository module can export `lazy(getDb, tagRepository)` as its default, built on first use and kept per handle: server functions and API handlers import that and never see the handle (decisions.md row 87). Only tests and the CLIs call the factory with a handle of their own. The `import.meta.glob` that bundles the SQL sits inside `bundledMigrations()` because the `seed` and `dev:seed` CLIs import repositories under plain Bun, where the glob does not exist.
