...
src/
  ...
  db/models/    one folder per domain, each with schema.ts (its Drizzle tables) and repo.ts (its repository: the factory by name, and as the default export the same repository over the application database — a lazy object built on first use, one per handle; recipe/ and shopping/ compose theirs from a context.ts, read.ts and write.ts beside it); columns.ts holds the shared column defaults and the conventions they all follow
