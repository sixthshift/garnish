...
src/
  ...
  domain/       what the household owns, one folder per thing, and every folder is a module: its index.ts is its whole surface, nothing outside the folder imports anything else in it (test/modules.test.ts), and the surface is what the app needs, not everything the folder has. Pure, no IO, importable by the client. Dependencies run one way — reference ← ingredient ← recipe ← shopping ← plan; import reads ingredient; draft reads recipe, ingredient, reference and import; style stands alone — so no two modules import each other. A screen's helper is not a domain: it lives with its screen or in lib/. At the root only what several domains share: names.ts (names and slugs), ids.ts (random UUIDs), lists.ts (reorder, every item but one, seeded order, set-like add and remove, URL array params), image.ts (image types and sniffing), markdown.ts, ai.ts (reading a model's answer)
