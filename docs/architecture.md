...
- **Server routes** under `/api/*` only for callers outside the app, and later import hooks. Field names follow Mealie where the concept exists.
  ...
  - The `.json` in both is a path suffix, not a param: the file routes are named `export[.]json.ts` and `{$slug}[.]json.ts`, where `[.]` escapes the dot the file convention would otherwise split on and the braces end the param.
