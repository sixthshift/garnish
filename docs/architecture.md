...
- **Server routes** under `/api/*` only for callers outside the app, and later import hooks. Field names follow Mealie where the concept exists.
  ...
  - `POST /api/import/file`: multipart, the export in the `file` field, 100 MB cap. Reads a Mealie backup zip or one recipe's JSON (`src/domain/importMealie.ts`, decisions.md row 73) and answers `{ recipes: [...] }` for the review step; a Tandoor file answers 400 with "Tandoor import is not available yet" until M34.4. Nothing is written — the review is what decides and the editor's Save is what writes.
