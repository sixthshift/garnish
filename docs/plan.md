...
- **Step ownership.** An ingredient can point at the step that uses it: `ingredient.step_id`, nullable, same part. Stored, not inferred. Decision 22's two lists per part stand; this is a pointer on one of them. The name matcher from M26.1 becomes the suggestion engine that pre-fills the pointer on import and on request, never silently on save.
- **The step card.** One card per step: number, the ingredients that step owns, the text, its timers in the footer. The same component is the cook deck's step card in large type, so the page is the deck laid out and cook mode is the deck one at a time. The left column stays as the prep list. The chips, the floating Ingredients button, the timer chips inside prose and the long press all go.
...
Not in this stage, and not to be re-raised without a decisions row: splitting one ingredient's amount across two steps (Tandoor can; here a food used twice is two rows, each on its own step), step images, nutrition, a shopping list (the "Add to shopping list" button is designed in M30.4 so the list is built against a real button, but the list itself stays Later), and a DOM test environment (three stage 5 questions stand as one decision for Jason).

## M28 Step ownership

- [ ] **! M28.1 `ingredient.step_id`.** `src/db/migrations/004_step_ingredients.sql` adds `step_id TEXT REFERENCES step(id) ON DELETE SET NULL` to `ingredient`, with an index. The Drizzle table in `src/db/models/recipe/schema.ts` mirrors it and `test/db/drift.test.ts` agrees. The document (`src/domain/recipe.ts`) gains `stepId: id.nullable().default(null)` on the ingredient, in both read and write shapes. The repository writes a part's steps before its ingredients so the key resolves, and nulls a `stepId` that does not name a step in the same part rather than failing the save. `architecture.md`'s data model and document sections are updated; decisions.md row. Check: migrate a fresh database and an existing one; a round-trip test that a document with assignments reads back with them; a test that a dangling `stepId` reads back null; the drift test.
- [ ] **M28.2 Suggesting steps.** `src/domain/stepIngredients.ts` gains `suggestStepAssignments(part)`: for every ingredient in the part with `stepId` null, the first step whose text matches it by `ingredientsInStep`'s rules, or null; assigned rows are left alone. Pure. The URL import's `draftFromScraped` (M23.6) applies it to each part before handing over the draft, so an imported recipe arrives with its pointers filled for review. Check: a table test over a part with two matching steps (the first wins), a row already assigned (untouched), a row nothing matches (stays null), and a food named in no step; an import test that the draft carries the suggestions.
- [ ] **M28.3 The editor knows about steps.** The ingredient row (inline from `md`, and in the phone sheet) gains a "Step" `Select` over the part's steps by number and first words, with "None". A step's number bubble in `StepsEditor` shows a small count of the ingredients pointing at it. The part's ingredient header gains "Suggest steps", which runs M28.2 over the part and reports how many it filled. Removing a step nulls the rows that pointed at it, in the draft, before save. Check: render tests for the select in both widths; a test that deleting a step clears its pointers; a test that Suggest steps fills only null rows.

## M29 The step card

- [ ] **! M29.1 `StepCard`.** `src/components/StepCard.tsx`: a quiet card (border, no shadow) with the step number, the ingredients whose `stepId` is this step as `IngredientRow`s (ticks shared), the step text through `Markdown` with no decorator, and a footer holding one `TimerChip` per duration `durationsIn` finds (deduplicated by seconds). From `md` the card is two columns inside: ingredients narrow on the left, text right; below `md` the list sits above the text. A `size` prop, `page` or `cook`, sets the type scale. Tapping the text still ticks the step. `StepList` renders `StepCard`s; `StepRow`, `StepIngredientChips` and `decorateDurations` are deleted, and `Markdown`'s `decorate` prop goes with them if nothing else uses it. decisions.md row (the view page and cook mode share one card). Check: render tests at both sizes with two owned ingredients, none, and a duration; the tick and timer tests move to the card.
- [ ] **M29.2 Cook mode deals the same cards.** `CookCardView`'s step branch renders `StepCard size="cook"`. The per-part ingredients card at the front of each part stays, showing only the part's ingredients that no step owns, so nothing is listed twice; a part where every ingredient is owned gets no ingredients card. `nextPreview` and the finish card stay. Check: cook route render tests for an owned ingredient appearing on its step card and not on the ingredients card; a part with everything owned skipping the ingredients card.
- [ ] **M29.3 The prep list stays; the rest goes.** The left column keeps every ingredient of every part (it is the gather-everything list). The floating Ingredients button, `IngredientsSheet` and `useScrolledOff` are deleted with their tests. The Made this button and `TimelineList` come off this task's route edits (M30 moves them). Check: the full suite; a grep finds no `IngredientsSheet` or `useScrolledOff`.
