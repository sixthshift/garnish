...
The model, settled with Jason on 2026-09-12: **a recipe is made of parts, and that is it. Each part contains its ingredients and its steps. Steps link to ingredients.** Ownership is the tree and has one answer: where a row lives, how much to buy, what the prep list shows. Linking is not the tree: a step may link any ingredient in the recipe, in any part, and an ingredient may be linked from several steps. Butter lives in the pastry and is used in step 1 and step 4. Nothing about the tree changes; decisions 22 and 49 stand.
...
- [ ] **! M28.1 `step_ingredient`.** `src/db/migrations/004_step_links.sql` creates `step_ingredient (step_id REFERENCES step ON DELETE CASCADE, ingredient_id REFERENCES ingredient ON DELETE CASCADE, PRIMARY KEY (step_id, ingredient_id))` with an index on `ingredient_id`. The Drizzle table in `src/db/models/recipe/schema.ts` mirrors it and `test/db/drift.test.ts` agrees. The document (`src/domain/recipe.ts`) gains `ingredientIds: z.array(id).default([])` on the step, in both read and write shapes, in link order. The repository writes every part's ingredients, then every part's steps, then the links, so an id from any part resolves; a link naming an ingredient not in this recipe is dropped rather than failing the save. `architecture.md`'s data model and document sections are updated; decisions.md row (steps link ingredients, many to many, any part). Check: migrate a fresh database and an existing one; a round-trip test that a step linking rows from two parts reads back with both; a test that a link to a foreign id reads back dropped; the drift test.
- [ ] **M28.2 Suggesting links.** `src/domain/stepIngredients.ts` gains `suggestLinks(recipe)`: for every step with no links, the ingredients across the whole recipe that `ingredientsInStep`'s rules find in its text, in list order; steps that already have links are left alone. Pure. The URL import's `draftFromScraped` (M23.6) applies it before handing over the draft, so an imported recipe arrives with its links filled for review. Check: a table test over a step naming two rows (both linked), a row named in two steps (linked from both), a step already linked (untouched), a step naming nothing (stays empty), and a food in another part (linked); an import test that the draft carries the suggestions.
- [ ] **M28.3 The editor knows about links.** Each step row in `StepsEditor` gains an "Ingredients" picker: a `Combobox` over every ingredient in the recipe by its formatted line, prefixed with the part name when the recipe has more than one; chosen rows show as removable chips under the step's textarea. The part's step header gains "Suggest links", which runs M28.2 over the recipe's steps and reports how many it filled. Deleting an ingredient row removes it from every step's links, in the draft, before save. Check: render tests for the picker and chips; a test that deleting a row clears its links; a test that Suggest links fills only unlinked steps.
