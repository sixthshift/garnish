// The uploaded Mealie export (M34.3). A route rather than a server function,
...
import { IMPORT_FIELD, readMealieExport, type MealieRecipe } from "../domain/importMealie";

export { IMPORT_FIELD } from "../domain/importMealie";

/** Largest export accepted. A household's whole Mealie backup is far under this. */
export const MAX_IMPORT_BYTES = 100 * 1024 * 1024;

/** What the route answers with. */
export type ImportFileResult = { recipes: MealieRecipe[] };
 ...
 * reads, with its image as a `data:` URL when the zip carried one. 400 with a
...
export async function handleImportFile(request: Request): Promise<Response> {
  ...
  try {
    const recipes = await readMealieExport({ name, bytes });
