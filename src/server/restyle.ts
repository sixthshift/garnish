...
// than copied: one `AiImportError` means the restyle sheet can show a failure
...
import { checkRestyle, type RestyleCheck, type RestyledPart } from "../domain/restyleCheck";
import {
...
export { AiImportError } from "./aiImport";
 ...
 * can rewrite them. `test/server/restyle.test.ts` holds it to the zod schema.
 */
...
/** The runner used in anger here: the import's fetch runner, told to ask for this schema and this model. */
 ...
 * `AiImportError("malformed", …)` for anything that is not an answer. Pure.
 */
export function parseRestyleAnswer(content: string): RestyledPart[] {
  ...
  try {
    ...
    throw new AiImportError("malformed", "The model answered in prose rather than the restyle format, so nothing was changed.");
  }
  ...
  if (!parsed.success) {
    ...
    throw new AiImportError("malformed", `The model's answer was not in the expected shape${where}. Nothing was changed.`);
  }
...
export async function runRestyle(
  ...
  let content: string;
  try {
    content = await run(prompt, AI_IMPORT_TIMEOUT_MS);
  } catch (cause) {
